...
def usage_stats(data, filters = [], verbose = True):
    ...
    #Computing the most common start and end hout
    start_hour = data['start_hour'].value_counts()
    max_start_hour = start_hour.max()
    #computing the number of trips on weekdays and weekends
    weekday = 0
    weekend = 0
    ...
    #computing the number of trips in each city of the data set

    city_names = data.start_city.unique()
    cities = np.array(city_names).tolist()
    city_data = {}
    for city in cities:
        city_data[city] = len(data[data['start_city'] == city]) 

    city_data_srt = sorted(city_data.items(), key=operator.itemgetter(1),reverse = True)
    ...
    if verbose:
        ...
        print('Number of trips on weekdays were {} compared to {} on weekends'.format(weekday, weekend))
        print('Number of trips per city :')
        print(city_data_srt)
